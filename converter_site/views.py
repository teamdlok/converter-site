...
def download_video(request):
    ...
    stream.download(output_path='./files/youtube_downloaded_dir', filename=file_name, skip_existing=False)
    return FileResponse(open(f'.//files/youtube_downloaded_dir/{file_name}', 'rb'), as_attachment=True)
...
def convertation_video(request):
    ...
    if request.method == 'POST' and request.FILES['file_for_convertation']:
        if request.FILES['file_for_convertation'].size > 100000000:
        ...
        if len(file_path_splited) > 1:
            file_path_splited_string = ''
            for i in range(0, len(file_path_splited)-1):
                file_path_splited_string = file_path_splited_string + file_path_splited[i]
        # Тут мы аккуратно получаем абсолютный путь к файлу, обрезая формат файла
        ...
        file_path_splited_output = file_path_splited_string.split('\input')
        file_path_splited_output = f"{file_path_splited_output[0]}\output{file_path_splited_output[1]}"
...
def convertation_picture(request):
    if request.method == 'POST' and request.FILES.get('file_for_convertation', False):
        if request.FILES['file_for_convertation'].size > 10000000:
...
def pictures_to_pdf(request):
    if request.method == 'POST' and request.FILES['pictures_for_pdf']:
        filename_list = []
        if request.FILES['pictures_for_pdf'].size > 10000000:
